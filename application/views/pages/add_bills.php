		...
		<div class="card  mb-4">
			<div class="card-body">
						...
						<div class="card  mb-4">
							...
							<div class="card-body">
								<div class="row">
									...
									<div class="col-sm-7 col-md-7" id="idBillNo">BL-001</div>
								</div>
								<div class="row">
									<div class="col-sm-5 col-md-5"><b>Bill Date</b></div>
									<div class="col-sm-7 col-md-7" id="idBillDate">07-11-2018</div>
								</div>
								<hr>
								<div class="row">
				...
				<div class="text-right">
					<button type="button" class="btn btn-success btn-sm" onclick="saveBill()">Save Bill</button>
...
<script type="text/javascript">
		...
		if(billOBJ['bill_items'].length > 0){
			$.each(billOBJ['bill_items'], function(indx, val){
				if(val.iProductId == iProductId && val.sItmUnit == sItmUnit){
					billOBJ['bill_items'][indx]['itemQty'] +=  itemQty;
	...
	function setSubTotal(){
		...
		if(COMPANY_STATE_CODE == iCustomerStateCode){
			if(parseFloat(fBillSubTotal) > 0){
				fBillCGST = (parseFloat(fBillSubTotal) * parseInt(COMPANY_CGST_TAX)) / 100.00;
				fBillSGST = (parseFloat(fBillSubTotal) * parseInt(COMPANY_SGST_TAX)) / 100.00;
	...
	function saveBill(){
		let url = "<?php echo base_url(); ?>pages/saveBill";
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.success_msg){
					...
					// location.href = "< ?php echo base_url(); ?>pages/loadView/bills";
					let iBLID = parseInt(data.iBillID);
					if( iBLID > 0){
						let url = "<?php echo base_url(); ?>pages/updateBillPage/"+iBLID+"/1";
						location.href = url;
					}
			...
			error: function(error) {
				showToastAlert("red", "fa fa-times", "Error", `Unable to add bill. Error: ${error}`);
			}
		});
	}
